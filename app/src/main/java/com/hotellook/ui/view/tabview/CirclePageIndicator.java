...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    static class SavedState extends BaseSavedState {
        ...
        /* renamed from: com.hotellook.ui.view.tabview.CirclePageIndicator.SavedState.1 */
        static class C14461 implements Creator<SavedState> {
            C14461() {
            }
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs) {
        ...
        if (!isInEditMode()) {
            ...
            int defaultPageColor = res.getColor(C1178R.color.default_circle_indicator_page_color);
            int defaultFillColor = res.getColor(C1178R.color.default_circle_indicator_fill_color);
            int defaultOrientation = res.getInteger(C1178R.integer.default_circle_indicator_orientation);
            int defaultStrokeColor = res.getColor(C1178R.color.default_circle_indicator_stroke_color);
            float defaultStrokeWidth = res.getDimension(C1178R.dimen.default_circle_indicator_stroke_width);
            float defaultRadius = res.getDimension(C1178R.dimen.default_circle_indicator_radius);
            boolean defaultCentered = res.getBoolean(C1178R.bool.default_circle_indicator_centered);
            boolean defaultSnap = res.getBoolean(C1178R.bool.default_circle_indicator_snap);
            int defaultOffsetFactor = res.getInteger(C1178R.integer.default_offset_factor);
            TypedArray a = context.obtainStyledAttributes(attrs, C1178R.styleable.CirclePageIndicator);
            ...
            this.mPaintStroke.setStrokeWidth(a.getDimension(8, defaultStrokeWidth));
            ...
            this.mOffsetFactor = a.getInt(9, defaultOffsetFactor);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mViewPager != null) {
            ...
            if (count != 0) {
                ...
                float offset = this.mRadius * ((float) this.mOffsetFactor);
                float shortOffset = ((float) shortPaddingBefore) + this.mRadius;
                float longOffset = ((float) longPaddingBefore) + this.mRadius;
                if (this.mCentered) {
                    longOffset += (((float) ((longSize - longPaddingBefore) - longPaddingAfter)) / 2.0f) - (((((float) count) * offset) - (offset / 2.0f)) / 2.0f);
                }
                ...
                for (int iLoop = 0; iLoop < count; iLoop++) {
                    float drawLong = longOffset + (((float) iLoop) * offset);
                    if (this.mOrientation == 0) {
                        dX = drawLong;
                        dY = shortOffset;
                    } else {
                ...
                float cx = ((float) (this.mSnap ? this.mSnapPage : this.mCurrentPage)) * offset;
                if (!this.mSnap) {
                    cx += this.mPageOffset * offset;
                }
                if (this.mOrientation == 0) {
                    dX = longOffset + cx;
                    dY = shortOffset;
    ...
    private int measureLong(int measureSpec) {
        ...
        if (specMode == RtlSpacingHelper.UNDEFINED) {
